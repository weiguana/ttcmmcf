...
Meteor.publish('adminAccountsIndexTabular', function (tableName, ids, fields) {
    ...
    fields.services = 1;
    fields.roles = 1;
    const usersHandle = Meteor.users.find({ _id: { $in: ids } }, { fields: fields }).observe({
        added: function (user) {
    ...
    if (Roles._collection) {
        const rolesHandle = Roles._collection.find({ userId: { $in: ids } }).observe({
            added: function (role) {
...
Meteor.methods({
    updateRoles: function (userId, roles) {
        ...
        const allowed = _.union.apply(this, Roles.helper(this.userId, 'accounts.allowedRoles'));
        const denied = _.union.apply(this, Roles.helper(this.userId, 'accounts.deniedRoles'));
        const finalRoles = _.difference(allowed, denied);
