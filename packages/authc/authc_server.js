...
//   var curUserId = doc._id.ops[0]._id;
    const curUserId = doc._id;

    if (ttcmmcf.adminExists) {
        // if there is a admin created we will set the default roles.
        const defaultRoles = Options.get('defaultRoles');
...
if (Roles._collection) {
    ttcmmcf.adminExists = Roles._collection.find({ roles: 'admin' }).count() !== 0;
} else {
    ttcmmcf.adminExists = Meteor.users.find({ roles: 'admin' }).count() !== 0;
}

Inject.obj('adminExists', { exists: !!ttcmmcf.adminExists });
